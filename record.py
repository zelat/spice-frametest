...
from gi.repository import SpiceClientGtk,SpiceClientGLib,Gtk,GObject,GLib
...
class SpiceRecorder(Viewer):
    ...
    def _channel_new_cb(self, session, channel):
        ...
        GObject.GObject.connect(channel, "open-fd", self._channel_open_fd_request)
    ...
    def _new_main_channel(self, session, channel):
        self._main_channel = channel
        self._main_channel.connect_after("channel-event", self._main_channel_event_cb)

    def _new_display_channel(self, session, channel):
...
def domain_extract_connect_info(domain):
    ...
    newtree = ET.ElementTree(tree)
    gfx = newtree.findall('devices/graphics')[1]
...
def _record(args, dom, tmpdir):
    ...
    with TtyCbreakMode():
        ...
        def add_display_widget_cb(sp, display):
            win.add(display)
            #win.fullscreen()

        def grab_keys_pressed_cb(sp):
            win.unfullscreen()
        ...
        #sp.run()
        sp.connect("add-display-widget", add_display_widget_cb)
        sp.connect("grab-keys-pressed", grab_keys_pressed_cb)
